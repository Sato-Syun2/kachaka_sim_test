...
public:
  ...
  : Node("mutual_sfm_node")
  {
    ...
    param_subscriber_ = this->add_on_set_parameters_callback(
      std::bind(&MutualSFMNode::parameters_callback, this, std::placeholders::_1));
...
private:
  ...
  void update_parameters() {
    ...
    if (new_situation != config_.situation_id) {
      ...
      reset_simulation(); // シチュエーションが変わったら即座にリセット
    }
  }
  ...
  void timer_callback()
  {
    ...
    msg.data = {elapsed_time, min_dist_to_human_, min_dist_to_left_wall_, min_dist_to_right_wall_};
    result_pub_->publish(msg);
    
    RCLCPP_INFO(this->get_logger(), "=== シミュレーション結果を送信し、リセットしました ===");
    reset_simulation();
    }

    publish_marker(robot_pub_, robot_.x, robot_.y, 1, 0.0, 0.0, 0.8, config_.robot_size);
    publish_marker(human_pub_, human_.x, human_.y, 2, 0.0, 0.8, 0.0, config_.human_size);
  ...
  void publish_marker(rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr publisher,
    double x, double y, int marker_id, double r, double g, double b, double size)
  {
    auto marker = visualization_msgs::msg::Marker();
    marker.header.frame_id = "map"; marker.header.stamp = this->now();
    ...
    marker.scale.x = size * 2.0; 
    marker.scale.y = size * 2.0; 
    marker.scale.z = 1.0;
...
rcl_interfaces::msg::SetParametersResult parameters_callback(
    const std::vector<rclcpp::Parameter> & parameters)
  {
    ...
    for (const auto & param : parameters) {
      if (param.get_name() == "A_classic") {
        config_.A_classic = param.as_double();
      } else if (param.get_name() == "B_classic") {
        config_.B_classic = param.as_double();
      } else if (param.get_name() == "T_c_limit") {
        config_.T_c_limit = param.as_double();
      } else if (param.get_name() == "A_cp") {
        config_.A_cp = param.as_double();
      } else if (param.get_name() == "B_cp") {
        config_.B_cp = param.as_double();
      } else if (param.get_name() == "weight_rep") {
        config_.weight_rep = param.as_double();
      } else if (param.get_name() == "weight_oss") {
        config_.weight_oss = param.as_double();
      } else if (param.get_name() == "A_wall") {
        config_.A_wall = param.as_double();
      } else if (param.get_name() == "B_wall") {
        config_.B_wall = param.as_double();
      } else if (param.get_name() == "d_safe") {
        config_.d_safe = param.as_double();
      }
    }

    RCLCPP_INFO(this->get_logger(), "パラメータ更新完了 (%zu 個)", parameters.size());
    return result;
  }
